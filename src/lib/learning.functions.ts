...
export const submitAttempt = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const rows = submittedAnswers.map((a) => {
      const chosenNorm = normalizeChoice(a.chosen);
      const correctNorm = correctMap.get(a.mcqId) || null;
      const isCorrect =
        chosenNorm !== null && correctNorm !== null && correctNorm === chosenNorm;
    ...
    try {
      ...
      if (affected.length) {
        const wrongIds = affected
          .filter((a) => correctById.get(a.mcqId) !== normalizeChoice(a.chosen))
          .map((a) => a.mcqId);
        ...
        for (const a of affected) {
          const isWrong = correctById.get(a.mcqId) !== normalizeChoice(a.chosen);
          if (!isWrong) continue;
  ...
  .handler(async ({ data, context }) => {
    ...
    return (rows ?? []).map((r) => {
      ...
      return {
        ...
        correct_option: normalizeChoice(m?.correct_option) ?? null,
        explanation: m?.explanation ?? null,
      };
    });
  });
...
export const submitCustomExamAttempt = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      const affected = data.answers.filter((a) => a.chosen !== null);
      const wrongIds = affected
        .filter((a) => correctMap.get(a.mcqId) !== normalizeChoice(a.chosen))
        .map((a) => a.mcqId);
      const correctIds = affected
        .filter((a) => correctMap.get(a.mcqId) === normalizeChoice(a.chosen))
        .map((a) => a.mcqId);
      ...
      if (allIds.length) {
        ...
        for (const a of affected) {
          const isWrong = correctMap.get(a.mcqId) !== normalizeChoice(a.chosen);
          if (!isWrong) continue;
          const prev = existMap.get(a.mcqId);
